...
#define	MAX_SOCK_NUM		8	/**< Maximum number of sockets  */
...
#define __DEF_IINCHIP_MAP_TXBUF__ (COMMON_BASE + 0x8000) /* Internal Tx buffer address of the W5200 */
#define __DEF_IINCHIP_MAP_RXBUF__ (COMMON_BASE + 0xC000) /* Internal Rx buffer address of the W5200 */
...
/**
 @brief Unreachable IP register address in UDP mode (xxx reserved)
 */
//#define UIPR0			(COMMON_BASE + 0x002A)
/**
 @brief Unreachable Port register address in UDP mode (xxx reserved)
 */
...
/**
 @brief Keep Timer register (xxx reserved)
 */
...
//#define IR_UNREACH	0x40		/**< get the destination unreachable message in UDP sending ( xxx reserved) */
#define IR_PPPoE		0x20		/**< get the PPPoE close message */
//#define IR_MAGIC		0x10		/**< get the magic packet interrupt (xxx reserved) */
//#define IR_SOCK(ch)	(0x01 << ch) 	/**< check socket interrupt (xxx reserved) */


/* PSTATUS register values */
#define POWERSAVE		0x10
#define POWERDOWN		0x08
...
void	IINCHIP_init(void); // reset W5200 - First call to make
void	IINCHIP_sysinit(uint8_t * tx_size, uint8_t * rx_size); // setting tx/rx buf size
